...
plot(mh0b)
# we need a different burn-in. Once the burn-in is removed, the outputs are indistinguishable.
plot(window(mh0, start=5e3))
plot(window(mh0b, start=1e4))
...
par(mfrow=c(3,1))
acf(mh1) # autocorrelation plots
acf(mh2)
acf(mh3)
effectiveSize(mh1) # effective sample sizes
effectiveSize(mh2)
effectiveSize(mh3)
raftery.diag(mh3)
...
# Question 11: extending the model to allow for some error on both direction and strength
prob_mod3 = function(sigma, x, tau) prob_mod2(sigma, x) * (2*pnorm(3, tau) -1)

logpost_mod3 = function(par, d = golf){
  sigma = par[1]
  tau = par[2]
  if(sigma<=0 | tau <=0) return(-Inf)
  
  p = prob_mod3(sigma, d$distance, tau)
  y = d$success
  llkd = sum(y * log(p) + (1-y) * log(1-p)) 
  logprior = logprior_mod2(sigma) + 2 * dnorm(tau, 1, log=T)
  if(is.na(llkd + logprior)) return(-Inf)
  return(llkd + logprior)
}

mh_mod3 = MCMCmetrop1R(logpost_mod3, c(1,1), tune=3, burnin=4000, mcmc=5000)
plot(mh_mod3)

plot(2:20, prop, ylim=c(0,1)) # success probability

sigma_mod3 = mean(mh_mod3[, 1])
tau_mod3 = mean(mh_mod3[, 2])
curve(prob_mod3(sigma_mod3, x, tau_mod3), add=T, col=5)

pred_mod2 = prob_mod2(sigma, 2:20)
mean((pred_mod2 - prop)^2)
pred_mod3 = prob_mod3(sigma_mod3, 2:20, tau_mod3)
mean((pred_mod3 - prop)^2)
